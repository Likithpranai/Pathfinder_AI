...
import { getAICareerRecommendations } from '../services/HuggingFaceService';
import AIPersonalityResultsScreen from '../components/AIPersonalityResultsScreen';
import { HUGGINGFACE_API_KEY, isHuggingFaceConfigured } from '../config/apiKeys';
import { LinearGradient } from 'expo-linear-gradient';

/**
 * This is a demo page to showcase the AI-powered career recommendations
 * It uses pre-filled sample answers to simulate a completed questionnaire
 */
export default function AICareerDemoScreen() {
  ...
  const processAnswers = async () => {
    ...
    if (!isHuggingFaceConfigured()) {
      Alert.alert(
        "API Key Required",
        "Please add your Hugging Face API key in the config/apiKeys.ts file to get AI-powered career recommendations.",
        [{ text: "OK" }]
      );
      return;
    }
    ...
    try {
      ...
      const response = await getAICareerRecommendations(sampleAnswers, HUGGINGFACE_API_KEY);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        <Text style={styles.title}>AI Career Recommendations</Text>
        <Text style={styles.subtitle}>
          Powered by Hugging Face AI
        </Text>
        <Text style={styles.description}>
          This demo shows how AI analyzes your questionnaire answers and suggests personalized career paths.
          We've pre-filled some sample answers that represent a tech-oriented person
          with creative secondary traits.
        </Text>
